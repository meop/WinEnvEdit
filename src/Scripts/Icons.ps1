...
# Generate variations
Write-Host "Generating variations:" -ForegroundColor Yellow
...
foreach ($var in $variations) {
    ...
    Write-Host "  Created: $($var.Name) ($($var.Width)x$($var.Height))" -ForegroundColor Cyan
}
...
Write-Host "  Created: app.ico ($($icoSizes.Count) resolutions)" -ForegroundColor Cyan

# Cleanup
$sourceImage.Dispose()

Write-Host "Generation complete. Created $($variations.Count + 1) files." -ForegroundColor Green
